...
from pareto_clustering.point_location.geo.spatial import toNumpy, convexHull
	...
	def _build_S(self, visualize):
		...
		for i in range(self.M):
			...
			for j in range(self.M):
				...
				tri_points_pair_orig = np.empty((6,self.num_iter))
				tri_points_pair_shuf = np.empty((6,self.num_iter))
				...
				for n in loop_over:
					...
					tri_points_pair_orig[:,n-t] = toNumpy(min_tri_s_orig.points).reshape((6,1)).T
					tri_points_pair_shuf[:,n-t] = toNumpy(min_tri_s_shuf.points).reshape((6,1)).T
				...
				if visualize:
					pair_triangles_orig[(i,j)] = tri_points_pair_orig.T.reshape(-1,2)
				print "\t%d(%d)" % (j,t),
			print
