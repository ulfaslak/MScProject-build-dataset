    ...
    def _compute_outliers(self):
        ...
        for i, combo in enumerate(self.component_combinations):
            ...
            for j,_ in enumerate(y_pred):
                pred_ys[j].append(y_pred[j])

        if self.hard:
            pred_avgs = dict((k,np.min(v)) for k,v in pred_ys.items())
        else:
            pred_avgs = dict((k,np.mean(v)) for k,v in pred_ys.items())

        outliers = [k for k,v in pred_avgs.items() if v < self.threshold]
...
class Outlier_detector_kd:
    """Compute outliers in X matrix
        Compute Kernel Density for each datapoint and mark outliers as
        point below threshold density.
    ...
    def __init__(self, X, visualize=False, threshold=-0.5,  bandwidth=1.0, kernel='gaussian'):
        ...
        self.clf = neighbors.KernelDensity(bandwidth=bandwidth, kernel=kernel)
